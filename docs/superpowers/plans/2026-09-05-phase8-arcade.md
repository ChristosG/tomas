...
- [ ] Each game is a composable `XGame(sizeDp: Float, onResult: (hits: Int, misses: Int, newSizeDp: Float) -> Unit, onSkip: () -> Unit)` that owns its round and calls `onResult` once at the end:
  ...
  - **PinchGame**: one of his item photos (random `imagePath != null`, else the app's ARASAAC pictogram of "μπάλα") with `detectTransformGestures` scaling 1×–3×; reaching ≥ 2× then returning ≤ 1.2× = hit; 3 rounds; instruction "Άνοιξε με δύο δάχτυλα, μετά κλείσε."
