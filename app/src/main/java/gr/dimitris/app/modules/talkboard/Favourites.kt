...
object Favourites {
    fun rank(all: List<Item>, pinnedIds: Set<String>, usage: List<ItemCount>, limit: Int = 12): List<Item> {
        ...
        // would see, so it opens on the start of the vocabulary instead. Quick phrases are left out —
        // the quick row above the tabs already shows every one of them.
        if (pinned.isEmpty() && used.isEmpty()) {
            return all.filter { it.category != Category.QUICK }
                .sortedWith(compareBy({ it.category.ordinal }, { it.text }))
                .take(limit)
        }
