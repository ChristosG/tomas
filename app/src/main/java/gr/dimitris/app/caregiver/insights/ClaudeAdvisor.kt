...
class ClaudeAdvisor(private val secrets: Secrets, private val model: suspend () -> String) {
    ...
    suspend fun ask(summary: String): Result<Advice> = withContext(Dispatchers.IO) {
        ...
        val chosen = runCatching { model() }.getOrNull()?.takeIf { it.isNotBlank() } ?: FALLBACK_MODEL
        ...
        try {
            ...
            val refused = response.stopReason().orElse(null)?.asString().equals("refusal", ignoreCase = true)
            if (refused) return@withContext Result.failure(AdviceException(REFUSED))
    ...
    companion object {
        ...
        /** Enough for the two short sections plus the thinking that leads to them. */
        ...
        fun parse(text: String): Advice {
            ...
            return Advice(
                ...
                dimitris = text.substring(d + DIMITRIS.length).trim(),
            )
        }
    }
}
